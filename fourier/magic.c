...
void	apple_init(const int _N, const int samplespersec, const int _samplelen, const int _bitspersample, const int _numchannels);
...
//	assert(FFT2PCM != NULL);
	...
	if (!inX)
		apple_init(N/2, samplespersec, samplelen, bitspersample, numchannels);
	else
